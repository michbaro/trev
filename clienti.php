    ...
    try {
        if ($_POST['action'] === 'save_client') {
            ...
            if ($id === null) {
                ...
                $stmt = $pdo->prepare('UPDATE cliente SET nome = :n WHERE id = :id');
        ...
        // FK su vendita.cliente → cliente.id
        echo json_encode([
            'ok'=>false,
            'message' => $e->getCode()==='23000'
                ? 'Impossibile eliminare: cliente collegato ad altre vendite.'
                : 'Errore database.'
        ]);
        exit;
    } catch (Throwable $e) {
...
try {
    $clienti = $pdo->query('SELECT id, nome FROM cliente ORDER BY nome ASC')->fetchAll();
} catch (Throwable $e) {
    $clienti = [];
}
?>
...
.actions-group{
  ...
  transform: translateX(90px) !important; /* stessa traslazione che usi negli altri file */
}
...
<div class="container">
  <div class="header">
    ...
    <div class="actions">
      ...
      <button class="btn primary" id="btnNew"><i class="bi bi-plus-lg"></i> Nuovo Cliente</button>
    </div>
  </div>
  ...
  <div class="card">
    <table class="table" id="clientTable">
      ...
      <?php foreach ($clienti as $c): ?>
        <tr data-id="<?= (int)$c['id'] ?>" data-nome="<?= htmlspecialchars($c['nome'], ENT_QUOTES) ?>">
    ...
    <?php if (empty($clienti)): ?>
      <div style="padding:16px"><span class="muted">Nessun cliente presente.</span></div>
    <?php endif; ?>
  </div>
</div>
  ...
  <div class="modal-wrapper">
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
      <div class="modal-header">
        <div class="modal-title" id="modalTitle">Nuovo Cliente</div>
      ...
      <form id="clientForm" class="modal-body">
        ...
        <div class="field">
          ...
          <input type="text" id="clientNome" name="nome" maxlength="255" required placeholder="Es. Azienda Agricola Verdi">
        </div>
      </form>
...
(function(){
  ...
  function openModal(mode,data){
    modalTitle.textContent = mode==='edit'?'Modifica Cliente':'Nuovo Cliente';
    if(mode==='edit'&&data){ clientId.value=data.id; clientNome.value=data.nome; }
    else{ clientId.value=''; clientNome.value=''; }
  ...
  (function(){
    ...
    if(p.get('success')==='1'){
      ...
      if(op==='added') msg='Cliente aggiunto';
      if(op==='updated') msg='Cliente aggiornato';
      if(op==='deleted') msg='Cliente eliminato';
  ...
  tbody.addEventListener('click',(e)=>{
    ...
    if(e.target.closest('.btn-del')){
      ...
      const fd=new FormData(); fd.append('action','delete_client'); fd.append('id',tr.dataset.id);
  ...
  btnSave.addEventListener('click',(e)=>{
    e.preventDefault();
    if(!clientNome.value.trim()){ clientNome.focus(); return; }
